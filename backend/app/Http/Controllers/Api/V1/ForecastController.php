...
class ForecastController extends Controller
{
    public function index(): JsonResponse
    {
        ...
        $byRep = Deal::where('status', 'open')
            ->join('users', 'users.id', '=', 'deals.owner_id')
            ->select('users.name', DB::raw('count(*) as deals'), DB::raw('sum(deals.value) as total'), DB::raw('sum(deals.value * COALESCE(deals.probability, 0) / 100) as weighted'))
            ->groupBy('users.name')
            ->get();
