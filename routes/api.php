...
/*
|--------------------------------------------------------------------------
| Third-party API (Sanctum tokens / API keys)
|--------------------------------------------------------------------------
|
| These routes serve consumers *outside* this application's own UI: mobile
| clients (`sync/delta`), API-key integrations (`api-keys`), the outbound
| webhook ecosystem, SSO provider callbacks, and the published OpenAPI spec.
| They are authenticated with tokens, not sessions.
|
| First-party pages are served by Inertia from routes/web.php and must not be
| duplicated here.
|
*/
